...
C) Examinando la dirección MAC de origen de cada trama recibida

D) Consultando un servidor DHCP central que le proporciona la tabla MAC

<details>
<summary>Respuesta</summary>
✅ Correcta: C) Examinando la dirección MAC de origen de cada trama recibida
...
💡 Explicación: Estos son los significados estándar convencionales. Nota: Cisco advierte que los colores pueden variar según el modelo, por lo que siempre se debe consultar el manual del equipo específico. El LED rojo fijo (no incluido aquí) indica fallo grave.
</details>
...
Se muestra un rack de comunicaciones con los siguientes equipos montados de arriba a abajo:
...
Se muestra una red con tres dispositivos (Host A, Switch S1, Router R1) y la siguiente tabla MAC del switch S1:
