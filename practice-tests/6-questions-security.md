...
D) Almacena el paquete hasta que el servidor interno responda
...
B) WPA Enterprise usa un servidor RADIUS para autenticar a cada usuario individualmente, por lo que el atacante no conoce la información usada para generar la clave pública inicial
...
<respuesta>
✅ Correcta: B) WPA Enterprise usa un servidor RADIUS para autenticar a cada usuario individualmente, por lo que el atacante no conoce la información usada para generar la clave pública inicial
...
C) Permite solo el tráfico Telnet hacia 10.0.0.5 y bloquea todo lo demás
D) Bloquea todo el tráfico excepto Telnet
...
B) Claves de 256 bits, message integrity code (MIC) en cada paquete, y autenticación de usuarios/hosts
...
<respuesta>
✅ Correcta: B) Claves de 256 bits, message integrity code (MIC) en cada paquete, y autenticación de usuarios/hosts
